...
contract Purchase {
  ...
  /*
  Ensuer that `msg.value` is an even number.
  ...
  */
  constructor() payable {
   ...
   function abort() external onlySeller inState(State.Created) {
      ...
      // reentrancy-safe, because it is the last call
      // in this function and we already changed the state. 
   ...
   function refundSeller() external onlySeller inState(State.Release) {
    emit SellerRefunded();
    // It is important to change the state first because
    // otherwise, the contracts called using `send` below can call in again here
    state = State.Inactive;
